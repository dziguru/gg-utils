...
def write_frame(frame, name=None, con=None, flavor='sqlite', if_exists='fail'):
    ...
    if_exists:
        ...
        'truncate' - first truncate table, then append
    ...
    if if_exists=='truncate' and table_exists(name, con, flavor):
        cur = con.cursor()
...
def test_postgresql(name, testdf):
    ...
    pgcn = pg.connect("host='%s' dbname='%s' user='%s' password='%s'"%('localhost','kds','postgres','anscd11'))
